...
| --value | String | False | None | Value of the item (use `-` to read from stdin) |
| --plaintext | None | NA | NA | Add the value as plain text |
| --force | None | NA | NA | Overwrite the item if it already exists |
| --file | String | False | None | Import items from a YAML/JSON/ENV file. The file needs to be flat |
...
Successfully added/updated secret my_secret into the vault test.slv.yaml
