...
type EnvironmentConfigData struct {
	...
	RootEnvironment *Environment            `yaml:"root"`
...
func (envConfig *EnvironmentConfig) newEnvConfigData() error {
	...
	envConfig.data.Environments = make(map[string]*Environment)
...
func (envConfig *EnvironmentConfig) GetRootEnv() (environment Environment) {
	return *envConfig.data.RootEnvironment
}

func (envConfig *EnvironmentConfig) GetEnvironments() (environments []*Environment) {
...
func (envConfig *EnvironmentConfig) GetEnvironment(id string) (environment *Environment, err error) {
...
func (envConfig *EnvironmentConfig) SearchEnvironments(query string) (environments []*Environment) {
...
func (envConfig *EnvironmentConfig) NewEnvironment(name, email string, envType EnvType) (env *Environment, privateKey *crypto.PrivateKey, err error) {
	env, privateKey, err = New(name, email, envType)
	if err != nil {
		return
	}
...
func (envConfig *EnvironmentConfig) updateEnvironment(env *Environment) {
...
func (envConfig *EnvironmentConfig) AddEnvironment(envString string) (err error) {
	environment, err := EnvFromSLVFormat(envString)
	if err != nil {
		return
	}
